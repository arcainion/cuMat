...
- **2D/3D block-level spatial locality in kernel launch configs** — `Context.h`: `CUMAT_KERNEL_2D_LOOP` and `CUMAT_KERNEL_3D_LOOP` rewritten to use nested 2D/3D grid-stride loops with `blockIdx.y`/`blockIdx.z` and `threadIdx.y`/`threadIdx.z` (was: 1D flattened iteration). `createLaunchConfig2D`/`createLaunchConfig3D` now compute balanced 2D/3D block dimensions (sqrt/cbrt breakdown of `bestBlockSize`) and emit 2D/3D grid dimensions. Threads in the same block now share Y/Z spatial locality, improving L1 cache behavior. Added `#include <cmath>` for `std::sqrt`/`std::cbrt`.
...
- **Replaced hardcoded warp size `32` with `warpSize`** — `ReductionOps.h`: The warp reduction kernel used hardcoded `32` for warp indexing and butterfly starting offset (`16 = 32/2`). Replaced all references with CUDA built-in `warpSize` variable, making the kernel automatically adapt to non-32 warp sizes on future architectures.
- **Removed dead `#if 0` code block** — `Context.h:386-407`: Removed the `#if 0 ... #else ... #endif` wrapper in `createLaunchConfig1D` that preserved a legacy hardcoded-256-block-size implementation alongside the live occupancy-optimized code.
